...
)
{
    ...
    ddist = 1;
    timeElapsed = 1;
    shortestDistance = 9999999999999999999;
    while (currTime < time2)
    {
        currTime = time1.AddSeconds(timeElapsed);
        Eci satPos = sgp4.FindPosition(currTime);
        Eci obsPos(currTime, user_geo);
        
        currentDistance = sqrt(pow(obsPos.Position().x - satPos.Position().x, 2) + 
                                pow(obsPos.Position().y - satPos.Position().y, 2) + 
                                pow(obsPos.Position().z - satPos.Position().z, 2));
    ...
    while (current_time < end_time)
    {
        ...
        else if (found_aos && topo.elevation < 0.0)
        {
            ...
            pd.max_elevation = FindMaxElevation(
                    ...
                    aos_time,
                    los_time);
            pd.tca = FindTCA(user_geo, sgp4, aos_time, los_time);
...
int main2()
{
    CoordGeodetic geo(41.3913358, -73.9565737, 0.05);
    Tle tle("STARLINK-24             ",
        "1 44238U 19029D   22274.16932777  .00341326  00000+0  39968-2 0  9996",
        "2 44238  52.9996 326.9514 0013188   9.0539 351.0707 15.59806026186224");
    SGP4 sgp4(tle);
